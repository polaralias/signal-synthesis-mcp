...
import { hashToken, generateRandomString } from './services/security';
import { getMasterKeyBytes, getMasterKeyInfo, isMasterKeyPresent } from './security/masterKey';
...
export class SignalSynthesisServer {
  ...
  async start() {
    ...
    if (!isMasterKeyPresent()) {
      throw new Error('MASTER_KEY is required to start the server');
    }
    getMasterKeyBytes();
    await runMigrations();
    ...
    if (port) {
      ...
      app.get('/health', (req, res) => {
        if (!isMasterKeyPresent()) {
          res.status(500).json({ status: 'error', error: 'MASTER_KEY is not configured' });
          return;
        }
        res.json({ status: 'ok' });
      });
